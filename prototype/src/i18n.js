...
const zh = {
  ...
  status: {
    ...
    refreshRetained: "刷新失败 · 正在显示上次数据",
    storageFailed: "菜单栏数据已更新 · 小组件可能仍是旧数据",
    firstRefreshFailed: "首次刷新失败 · 还没有可显示的数据",
    noDataYet: "还没有可显示的数据",
    healthTitle: "运行状况",
    healthNote: "这些检查来自 agentdeck doctor，可在终端运行以查看修复建议。",
    copied: "已复制",
    copySyncCommand: "复制同步命令",
    copyDiagnosticCommand: "复制诊断命令",
    copySafetySteps: "复制安全步骤",
    manualPrerequisite: "人工前置：",
    healthRecoveryNotices: {
      stateLive: "AgentDeck 状态正在被占用",
      scanLive: "AgentDeck 扫描正在进行",
      legacy: "旧式状态锁需要人工确认",
      ownerUnknown: "无法确认扫描锁的归属",
      extensionStale: "扩展库存已过期",
      discoveryFailed: "扩展发现失败",
      syncIncomplete: "库存已更新，但扫描指纹未更新",
    },
    healthRecoveryDetails: {
      stateLive: { cause: "另一个 AgentDeck 状态操作仍在运行。", next: "等待该操作结束，然后重试失败的命令。" },
      scanLive: { cause: "另一个 AgentDeck 扫描仍在运行。", next: "等待扫描结束，然后重试失败的命令。" },
      lock_legacy: { cause: "此锁没有可验证的现代所有者信息。", next: "AgentDeck 不会自动删除它。", manual: "先确认没有 AgentDeck 进程正在使用此状态目录；确认后才可移除 state.lock。" },
      lock_owner_unknown: { cause: "AgentDeck 无法安全证明扫描锁是否仍有所有者。", next: "不要删除 scan.lock；通过正常方式停止所属进程，或等待后再次诊断。" },
      extension_stale_inventory: { cause: "成功的实时发现中已不存在 1 个已存扩展身份。", next: "同步 AgentDeck 的扩展库存，然后再次刷新健康状态。", effect: "只更新 AgentDeck 的派生扩展库存与扫描指纹；不修改客户端配置或已安装扩展。" },
      extension_discovery_failed: { cause: "原生扩展发现未完成，因此无法判断库存是否陈旧。", next: "先解决发现错误，再次刷新健康状态；此时不提供同步命令。" },
      extension_fingerprint_update_failed: { cause: "库存事务已经提交，但后续扫描指纹没有更新。", next: "先诊断已提交库存的健康状态，再决定是否重试同步。", effect: "客户端配置未改变，库存也没有回滚。" },
    },
    expand: "展开",
    collapse: "收起",
    checks: {
      ...
      price_catalog: "价格目录",
      session_index: "会话索引",
      state_permissions: "状态文件权限",
      state_lock: "状态锁",
      scan_lock: "扫描锁",
      extensions: "扩展库存",
      database: "核心数据库",
      hook_deliveries: "Hook 投递",
      state: "状态",
      schema: "数据库结构",
      database_integrity: "数据库完整性",
      pending_operations: "待处理操作",
      provider_operation_state: "服务商操作状态",
      provider_configuration: "服务商配置",
      provider_credentials: "服务商凭据",
      project_attribution_gate: "项目归因门禁",
      sessions: "会话索引",
      session_sources: "会话来源",
      usage: "用量索引",
      usage_sources: "用量来源",
      prices: "价格目录",
      price_provenance: "价格来源",
      unpriced_models: "未计价模型",
      other: "其他运行状况检查",
      affected: (count) => `受影响：${count} 项`,
    },
  ...
  widgets: {
    ...
    refresh: {
      ...
      old: "上次更新于8小时前",
      unchanged: "2 分钟前更新",
      missing: { title: "还没有小组件数据", footer: "打开 AgentDeck 以刷新" },
      containerUnavailable: { title: "小组件存储不可用", footer: "打开 AgentDeck 以重试" },
      readFailed: { title: "无法读取小组件数据", footer: "等待下次刷新重试" },
      unsupported: { title: "小组件数据来自较新版本的 AgentDeck", footer: "升级 AgentDeck 后刷新" },
    },
  },
  settings: {
    title: "AgentDeck 设置",
    ...
    periodicRefreshHint: "AgentDeck 运行时约每分钟刷新一次；关闭后，启动与手动刷新仍会更新数据",
    menubar: "菜单栏",
    menubarValue: "显示内容",
  ...
  states: {
    ...
    refresh: "刷新状态",
    healthRecovery: "健康恢复",
    healthRecoveryCliNote: "Text 是已交付 CLI 契约的逐字符标本；JSON 面板只冻结必需字段，最终容器由 architecture 决定。语言开关不改变 CLI 英文输出。",
    healthRecoveryVariants: {
      baseline: "原有健康状态",
      stateLive: "状态锁 · 活跃",
      scanLive: "扫描锁 · 活跃",
      legacy: "状态锁 · 旧式",
      ownerUnknown: "扫描锁 · 未知归属",
      extensionStale: "扩展库存陈旧",
      discoveryFailed: "扩展发现失败",
      syncIncomplete: "库存已提交 · 指纹失败",
      combined: "状态锁 + 库存陈旧",
    },
    widgetRefresh: "小组件刷新",
    widgetClient: "小号端",
    ...
    refreshVariants: {
      ...
      refreshing: "刷新中",
      failed: "失败后保留旧数据",
      storageFailed: "小组件发布失败",
      wake: "休眠后补刷",
      recovered: "已恢复",
      firstFailure: "首次刷新失败",
    },
    widgetRefreshVariants: {
      fresh: "新鲜",
      aging: "正在老化",
      old: "已过期",
      hostAbsent: "Host 缺席",
...
const en = {
  ...
  refreshRetryLabel: "Refresh failed. Retry",
  ...
  status: {
    ...
    refreshRetained: "Refresh failed · showing previous data",
    storageFailed: "Menu-bar data is current · Widgets may be out of date",
    firstRefreshFailed: "First refresh failed · no data is available yet",
    ...
    copied: "Copied",
    copySyncCommand: "Copy sync command",
    copyDiagnosticCommand: "Copy diagnostic command",
    copySafetySteps: "Copy safety steps",
    manualPrerequisite: "Manual prerequisite:",
    healthRecoveryNotices: {
      stateLive: "AgentDeck state is busy",
      scanLive: "An AgentDeck scan is running",
      legacy: "A legacy state lock needs confirmation",
      ownerUnknown: "Scan-lock ownership is unknown",
      extensionStale: "Extension inventory is out of date",
      discoveryFailed: "Extension discovery failed",
      syncIncomplete: "Inventory updated, but its scan fingerprint did not",
    },
    healthRecoveryDetails: {
      stateLive: { cause: "Another AgentDeck state operation is still running.", next: "Wait for it to finish, then retry the failed command." },
      scanLive: { cause: "Another AgentDeck scan is still running.", next: "Wait for the scan to finish, then retry the failed command." },
      lock_legacy: { cause: "This lock has no verifiable modern owner information.", next: "AgentDeck will not remove it automatically.", manual: "First confirm that no AgentDeck process is using this state directory; only then may you remove state.lock." },
      lock_owner_unknown: { cause: "AgentDeck cannot safely prove whether the scan lock still has an owner.", next: "Do not remove scan.lock; stop its process normally, or wait and diagnose again." },
      extension_stale_inventory: { cause: "One stored extension identity is absent from successful live discovery.", next: "Synchronize AgentDeck's extension inventory, then refresh health again.", effect: "Updates only AgentDeck's derived extension inventory and scan fingerprint; client configuration and installed extensions stay unchanged." },
      extension_discovery_failed: { cause: "Native extension discovery did not finish, so stale inventory cannot be determined.", next: "Resolve discovery first, then refresh health again; no sync command is offered now." },
      extension_fingerprint_update_failed: { cause: "The inventory transaction committed, but the later scan fingerprint did not update.", next: "Diagnose the committed inventory before deciding whether to retry sync.", effect: "Client configuration did not change, and inventory was not rolled back." },
    },
    expand: "Show",
    collapse: "Hide",
    checks: {
  ...
  widgets: {
    ...
    refresh: {
      aging: "Updated 18 minutes ago",
      old: "Last updated 8 hours ago",
      unchanged: "Updated 2 minutes ago",
      missing: { title: "No Widget data yet", footer: "Open AgentDeck to refresh" },
      containerUnavailable: { title: "Widget storage unavailable", footer: "Open AgentDeck to retry" },
      readFailed: { title: "Widget data could not be read", footer: "Will retry on the next refresh" },
      unsupported: { title: "Widget data is from a newer AgentDeck", footer: "Upgrade AgentDeck to refresh" },
    },
  },
  settings: {
    ...
    periodicRefreshHint: "Refreshes about once a minute while AgentDeck is running; when off, startup and manual refresh still update data",
    menubar: "Menu bar",
    menubarValue: "Shows",
  ...
  states: {
    ...
    healthRecoveryCliNote: "Text is byte-exact from the delivered CLI contract; the JSON panel freezes required fields while architecture owns the final container. The language switch does not localize CLI output.",
    healthRecoveryVariants: {
      baseline: "Existing health state",
      stateLive: "State lock · live",
      scanLive: "Scan lock · live",
      legacy: "State lock · legacy",
      ownerUnknown: "Scan lock · unknown owner",
      extensionStale: "Stale extension inventory",
      discoveryFailed: "Extension discovery failed",
      syncIncomplete: "Inventory committed · fingerprint failed",
      combined: "State lock + stale inventory",
    },
    widgetRefresh: "Widget refresh",
    ...
    refreshVariants: {
      idle: "Idle",
      refreshing: "Refreshing",
      failed: "Failed with prior data",
      storageFailed: "Widget publish failed",
      wake: "Catch-up after sleep",
      recovered: "Recovered",
      firstFailure: "First refresh failed",
    },
    widgetRefreshVariants: {
      fresh: "Fresh",
      aging: "Aging",
      old: "Old",
      hostAbsent: "Host absent",
      unchanged: "Unchanged publication",
      changed: "Changed publication",
