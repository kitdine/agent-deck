...
export function App() {
  ...
  const { lang, theme, state, width, quota, refresh, anchor } = stage;
  const dict = catalogs[lang];
  ...
  return (
    <main className="stage" data-theme={theme}>
      <StageControls prefs={stage} showAnchor showRefresh />
      ...
      <div className="stage-body" data-anchor={anchor}>
        ...
        {open && <Popover lang={lang} state={state} quotaState={quota} refreshScenario={refresh} width={width} scan={scan.enabled ? scan : null} onClientChange={setClient} />}
      </div>
      {settings && (
