...
`snapshot-complete.json`, `snapshot-partial.json`, `snapshot-empty-client.json`
and `snapshot-schema-ahead.json`
...
```bash
AGENTDECK_UPDATE_FIXTURES=1 scripts/run-go-test.sh ./internal/desktop -run TestCanonicalFixturesAreReproducibleProducerOutput
```
...
swift desktop/fixtures/v1/verify.swift \
  ...
  desktop/fixtures/v1/snapshot-schema-ahead.json
```
