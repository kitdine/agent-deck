...
schema 24, schema 12 reports a `schema_outdated` schema check with `count: 12`,
`supported_count: 24` and recovery command `agentdeck state migrate`. A complete
...
`supported_count: 24`, and no recovery command, in a partial report. These
