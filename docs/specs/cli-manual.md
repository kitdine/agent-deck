...
- **重启提示（每次 Claude 切换都有，按方向区分文案）**：AgentDeck 没有任何单个会话
  的鉴权状态，无法知道某个正在运行的会话此刻是否已经持有 key。因此提示按“本次完成
  的选择是否携带凭据”分两种保守文案，而不是笼统声称切换已经生效：
  - 完成的选择携带凭据时：只有一个未持有 key 就启动的会话可能实时采用它；已经用某
    个 key 完成鉴权的会话在重启前仍保持原 key。
  - 完成的选择不携带凭据时：删除 key 不会让已经持有它的会话重新鉴权，其他配置项是
    否被采用在重启前无法确定。
  两种文案都建议重启正在运行的 Claude 会话；它们只声明 AgentDeck 能确认的边界，不
  对某个具体正在运行的会话下结论。
