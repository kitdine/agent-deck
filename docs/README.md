...
| [CLI Error Classification](topics/cli-error-classification/tasks.md) | `v0.5.0` | Implementation complete — 2/2 tasks reviewed and committed with immutable Task gates VERIFIED; containing-unit CEv1 gate and v0.5.0 contract closure remain separate | Stable not-found codes, and no storage text in a documented JSON contract. Breaks the documented `runtime_error` value; announced in this version's notes. |
