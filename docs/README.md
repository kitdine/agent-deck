...
| [CLI Error Classification](topics/cli-error-classification/tasks.md) | `v0.5.0` | Active — all design documents PASS; task 1 `typed-not-found-errors` reviewed and committed with CEv1 Task gate VERIFIED; task 2 `stable-error-codes` next; 1/2 implementation tasks delivered | Stable not-found codes, and no storage text in a documented JSON contract. Breaks the documented `runtime_error` value; announced in this version's notes. |
