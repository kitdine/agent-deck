...
| [Work Signals](topics/work-signals/tasks.md) | `v0.5.0` | Active — prototype task 0 PASS and committed; all five design documents PASS with current-state gates VERIFIED; implementation task 1 PASS at independent Re-review Round 2 after a test-only repair closed R1-F1, delivering schema v20 / parser v5 with its CEv1 gate VERIFIED across all seven criteria and awaiting its commit checkpoint; task 2 `activity-classification` next; 1/6 implementation tasks reviewed | Activity classification, workflow metrics, and tool-call attribution, delivered on two first-class surfaces: the menu-bar `Sessions` panel's three pending modules and a new `agentdeck usage signals`. |
