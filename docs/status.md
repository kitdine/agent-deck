...
| [Switch Effectiveness Boundary](topics/switch-effectiveness-boundary/tasks.md) | `v0.5.0` | Active — requirements PASS on the corrected state machine; architecture and tasks await review; 0/4 draft implementation tasks | Only `no key -> first key` may update a running Claude session; key rotation, key removal, and other changes require restart and must retain the session's prior billing route. |
