...
| [Schema Version Signal](topics/schema-version-signal/tasks.md) | unassigned | Active — surface framework drafted; `requirements.md` and `ux/menubar-schema-signal.md` are 2/4 documents reviewed, PASS at Round 2 on 2026-09-06 after a Round 1 REOPEN over four findings, with its `document` CEv1 gate VERIFIED against `head=8d283cd`/`document=b0854d00`; `ux/menubar-schema-signal.md` was drafted 2026-09-06, its framework review returned FAIL at Round 1 on 2026-09-06 over two findings — the document carried hand-drawn ASCII specimens for a GUI surface and never referenced the root `prototype/`, which `prototype/README.md` declares the product's single design truth and which all four historical GUI `ux/*.md` documents source their specimens from, and D7 left the menu-bar badge predicate unassigned between the Shared `isBadged` and the App's `menuBarBadged` — and both were repaired and closed at Round 2 on 2026-09-06, which reached PASS with both required criteria of its own `document` CEv1 gate VERIFIED against the candidate state `head=fb8c6fb`/`document=bab2f89c`/`prototype=9d5faab6` — a three-part state because the repair moved the specimen out of the document, so a document-only fingerprint could no longer prove the specimen is still there and still this version. The re-review re-measured every number itself on a live prototype server rather than accepting the document's, including a mutation check proving the gauge can fail and a contrast check showing the footer string shipped today already overflows by 2 px at the narrow bound the attributed one clears by 37; it records that reviewer and repairer were the same session agent, so its weight rests on reproducible measurements rather than on independence. One out-of-scope pre-existing finding is carried on Beads `ad-bug-prototype-probe-pending-banner`: the prototype's `probe=1` self-check has reported `1 FAILED` since `151c6d3` because an assertion still expects the pending banner in the `normal` stage state, proven pre-existing by running the same check against a clean export of HEAD. The repaired document: the document now declares `/prototype/` as its specimen source with a prototype-wins clause, the prototype carries this condition as two new stage states (`schema` and `schemaStacked`) with the seven copy keys in both language dictionaries, and the 280 pt footer truncation the draft had deferred to a real render is now settled at the specimen stage — the prototype gained a 420/280 width switch matching `AGENTDECK_TEST_WIDTH` and its overflow gauge gained ellipsis-truncation detection, a blind spot its own README had recorded, after which both languages clear at 280 pt with 80.33 px and 37.14 px of headroom; D7 now assigns the badge predicate to the App layer, because `isBadged` describes refresh state while this condition is snapshot content; the draft resolves the widget question by declining to add a widget state (D8), on the ground that `WidgetDesktopSnapshotV1` decodes no `health`, and requesting two wire additions of `architecture.md` — the two schema version numbers on the health check, and an envelope-level cause so the four consequence warnings are suppressed by attribution rather than a hard-coded list; version membership not yet decided by a contract topic, so this row is deliberately not marked `v0.5.0`; the Documents matrix now declares only `architecture.md` as required and unwritten, which `check-topic-docs.sh` reports as a gap until `tasks.md` review ratifies the set; `requirements.md` was widened on 2026-09-04 while in review, after a triage found a sixth reporting path — the Hook — and it is the one that reports nothing at all | One stable, actionable report when the core database's schema version exceeds the running binary's supported version, across `doctor`, the command paths, the desktop snapshot, and Hook delivery. Promoted from a measured defect: schema 21 on disk against two installed binaries supporting 18, reported six different ways. The sixth is silent: `usage hook event` exits 0 with empty streams and writes no row, which cost 19 Codex sessions their provider route between 2026-08-30 and 2026-09-01, unrecoverably. |
