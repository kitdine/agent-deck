...
| [Usage Attribution Precision](topics/usage-attribution-precision/tasks.md) | `v0.5.0` | Active — tasks 1–2 reviewed and committed; task 3 `attribution-observability` PASS at Re-review Round 2 with its CEv1 gate VERIFIED across all nine criteria and awaiting its commit checkpoint; 3/3 implementation tasks reviewed; release blockers: no determinable event may be downgraded to `inferred`, and no unattributed event may enter provider spend | Effective-session attribution semantics, determinability-based quality, and an unattributed boundary that never enters a real-spend total. |
