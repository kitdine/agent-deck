...
| [Usage Attribution Precision](topics/usage-attribution-precision/tasks.md) | `v0.5.0` | Active — three design documents repaired for Review Round 1 findings (R1-F1..F3, A1-F1..F5, T1-F1..F3) and awaiting independent Re-review; 0/3 implementation tasks; release blocker: a determinable event must never be downgraded to `inferred` | Effective-session attribution semantics, determinability-based quality, and an unattributed boundary that never enters a real-spend total. |
