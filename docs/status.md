...
| [Usage Attribution Precision](topics/usage-attribution-precision/tasks.md) | `v0.5.0` | Active — all three design documents PASS at Re-review Round 3 (2026-08-26), CEv1 gates VERIFIED, design frozen and the three implementation tasks dispatchable; the reference classifier keeps 4 of 166 legacy route rows and 534/12,929 Claude events (4.13%) at `estimated`; 0/3 implementation tasks; release blocker: a determinable event must never be downgraded to `inferred` | Effective-session attribution semantics, determinability-based quality, and an unattributed boundary that never enters a real-spend total. |
