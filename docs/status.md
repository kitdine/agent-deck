...
| [Work Signals](topics/work-signals/tasks.md) | `v0.5.0` | Active — prototype task 0 and implementation tasks 1–3 PASS and committed; all five original design documents PASS at independent Re-review Round 8; task 4 `work-signal-projection` PASS and committed at `7a1160a` / tree `98a2ef6c`, with the immutable Task gate VERIFIED; task 5 `work-signal-surface` PASS at independent Re-review Round 1: R1-F1 closed on a falsifier — reinstating the unanimity rule made the new asymmetric-scope regression fail at all three boundaries — and R1-F2 closed by the `ux/session-work-signals.md` Round 9 ratification and a Document gate that reproduces from its own recipe; Task gate VERIFIED and awaiting commit; actual VoiceOver/TCC/system accessibility automation stays explicitly not run and not required; 5/6 implementation tasks reviewed | Activity classification, workflow metrics, and tool-call attribution, delivered on two first-class surfaces: the menu-bar `Sessions` panel's three pending modules and a new `agentdeck usage signals`. |
