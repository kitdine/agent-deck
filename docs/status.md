...
| [Usage Attribution Precision](topics/usage-attribution-precision/tasks.md) | `v0.5.0` | Active — all three design documents PASS with CEv1 gates VERIFIED; task 1 `client-time-semantics` PASS at Re-review Round 2 with its CEv1 gate VERIFIED across all five criteria, tasks 2–3 not started; 1/3 implementation tasks reviewed; release blocker: a determinable event must never be downgraded to `inferred` | Effective-session attribution semantics, determinability-based quality, and an unattributed boundary that never enters a real-spend total. |
