...
| [Usage Attribution Precision](topics/usage-attribution-precision/tasks.md) | `v0.5.0` | Active — task 1 reviewed and committed; task 2 `determinability-quality` PASS at Re-review Round 2 with its CEv1 gate VERIFIED across all seven criteria and awaiting its commit checkpoint, task 3 not started; 2/3 implementation tasks reviewed; release blocker: a determinable event must never be downgraded to `inferred` | Effective-session attribution semantics, determinability-based quality, and an unattributed boundary that never enters a real-spend total. |
