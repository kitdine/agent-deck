...
- `agentdeck extension adopt` refuses an inventory row while its native fingerprint is empty. Restoring the source and rescanning must establish a fingerprint before adoption can record management ownership; an unavailable row must not create an empty adoption fingerprint or a new management anomaly.
...
- When `fingerprint_sync_incomplete` is true, extension-doctor text prints the prior partial-commit cause and stale-health effect even if another reason is primary. For a marker-only report, its `next:` checks AgentDeck state permissions and then advises retrying scan and diagnosis; it does not repeat the already-running diagnostic command as a next step or add a machine `recovery_command`.
